...
//!
//! - El actor `Hotel` va a ser el encargado de simular el procesamiento de los request que incluyan una reserva de hotel además de la del vuelo. Al igual que la primera implementación, el pedido consta de esperar un tiempo al azar de no más de un segundo y medio. Como el actor `Hotel` solo puede ejecutar un mensaje a la vez, para permitir la ejecución de otros pedidos en simultáneo se hace uso de los *sleeps de tasks*(`actix::clock::sleep`) dentro de un `Box::pin` que le permite al handler devolver un `ResponseActFuture`, es decir que el actor `Hotel` no espera a que el sleep se complete pero una vez que finaliza, el actor a través de un callback se encarga de finalizar con el procesamiento. Como la simulación es siempre exitosa, cuando finaliza el sleep, simplemente envía sus estadísticas al actor `Statistics`.
//! 
//! - El actor `AirlineManager` cuenta con tres campos en su estructura: en primer lugar el `HashMap` para identificar la cantidad máxima de request que tiene permitido procesar cada aerolínea y la cantidad que está procesando en ese momento; la dirección del actor `Airline` para poder enviarle los pedidos de reserva; y un `HashMap` con una cola de vuelos que aún no se pueden procesar debido al rate limit de la aerolínea. Al contar con las colas de pedidos pendientes y el contador de pedidos ejecutándose, el manager resuelve las tareas de forma similar a un semáforo. El actor puede recibir dos tipos de mensaje: un mensaje `V` de tipo `NewRequest` en el que verifica si el rate limit de la aerolínea está disponible, si lo está, lo procesa(lo envía al actor `Airline`) y suma uno al contador, sino, lo agrega a la cola de pedidos pendientes; y un mensaje `P` de tipo `FinishRequest` que se encarga de restar en uno al contador y de verificar si hay algún pedido pendiente en la cola (si lo hay, lo procesa).
//!
//! - Una vez que **AirlineManager** resuelve el *rate limit*, el **InfoFlight** se envía al actor **Airline**, el cual resolverá el vuelo de la misma manera que lo hacía **Hotel** (con un `sleep` dentro de `Box::pin`). La única diferencia entre estos dos actores es que el de la aerolínea puede fallar. En este caso, el actor re-intentará el vuelo, enviando a sí mismo (con `actix::actor::AsyncContext::notify`) el **InfoFlight** recibido, pero marcando que es un reintento, así haciendo la espera del servidor simulado más larga (los segundos de penalización previos al reintento, y los segundos de la simulación).
//! 
