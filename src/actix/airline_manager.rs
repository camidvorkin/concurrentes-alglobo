use actix::{Actor, Context, Handler, Message, AsyncContext, Addr};
...
impl Handler<NewRequest> for AirlineManager {
    ...
    fn handle(&mut self, msg: NewRequest, ctx: &mut Self::Context) -> Self::Result {
        let webservice = self.requests_per_airline.entry(msg.info_flight.flight_reservation.airline.clone()).or_insert((0,0));
        let rate_limit= webservice.0;
        let current_amount_requests = webservice.1;
        
        if rate_limit > current_amount_requests {
            self.requests_per_airline.insert(msg.info_flight.flight_reservation.airline.clone(), (rate_limit, current_amount_requests + 1));
            self.addr_airline.try_send(msg.info_flight.clone());
        } else {
...
pub fn from_file(
    ...
    for airline in airlines {
        airline_map.insert(airline[0].to_string(), (airline[1].parse::<u64>().unwrap(), 0));
    }
    let addr_airline = Airline {
        addr_statistics: addr_statistics,
    }.start();
    AirlineManager {
        requests_per_airline: airline_map,
        addr_airline: addr_airline,
    }.start()
}
